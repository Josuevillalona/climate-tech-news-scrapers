...
def scrape_agfundernews_page(page_number):
    """Scrapes a specific page of the AgFunderNews archive."""
    ...
    article_headers = soup.select('h4.article-title a')
...
def get_article_text(article_url):
    ...
    article_body = soup.find('div', class_='article-content') or soup.find('div', class_='entry-content')
    if article_body:
...
def main():
    ...
    print("Starting AgFunderNews daily scraper (NEW SCHEMA)...")
    
    all_article_urls = []
    for i in range(1, PAGES_TO_SCRAPE + 1):
        page_urls = scrape_agfundernews_page(i)
        if not page_urls:
    ...
    print(f"\nFound a total of {len(all_article_urls)} article URLs from AgFunderNews.")
    ...
    print(f"\n🎉 AgFunderNews scrape complete! Added {new_articles_found} new articles using NEW SCHEMA.")
