...
def scrape_axios_page():
    """Scrapes the Axios climate newsletter page."""
    ...
    article_containers = soup.find_all(['article', 'div'], class_=['story', 'post', 'article'])
    for container in article_containers:
        link_tags = container.find_all('a', href=True)
        for link_tag in link_tags:
            href = link_tag['href']
            if '/newsletters/axios-generate/' in href or '/climate' in href:
...
def main():
    ...
    print("Starting Axios climate scraper (NEW SCHEMA)...")
    
    article_urls = scrape_axios_page()
    if not article_urls:
    ...
    print(f"\n🎉 Axios climate scrape complete! Added {new_articles_found} new articles using NEW SCHEMA.")
